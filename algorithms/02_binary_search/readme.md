...
(**舍弃一个端点，也是在缩小区间**)
时间复杂度O(logN)，最坏O(N)，当所有元素相等时，我们需要一个一个去排除右端点。空间复杂度O(1)。
...
## 287_Find_the_Duplicate_Number 二分查找
题目要求不能改变原数组，因此不能排序。这让我第一反应觉得无法使用二分查找，但是再想想，如果可以排序，二分查找并不能满足要求，还不如遍历。**因此，二分的对象一定不是数组本身，而是一个天然存在的有序结构。**

在1~n这个范围（1和n都存在），对于i，我们如果统计数组中小于等于i的数的个数count，那么就会发现，在重复的数之前，count<=i；在重复的数及其之后，count>i。
比如，对于数组$[1,3,2,2,4]$，有:

| nums | 1	| 2  | 3  | 4  |
| ---- |----|----|----|----|
| cnt  | 1	|3	 |4	  |  5 |
所以，我们在1~n上进行二分，对于每次得到的mid，统计数组中小于等于i的数的个数count。
```cpp
if(count<=mid) low = mid + 1;
else high = mid;
```
* 时间复杂度O(NlogN)，二分查找最多需要二分 O(logn) 次，每次判断的时候需要O(n) 遍历nums[]数组求解小于等于 mid 的数的个数。

* 空间复杂度O(1)。

## 4_Median_of_Two_Sorted_Arrays
原地根据下标直接找到中位数，即使用双指针，容易实现，但是不符合时间复杂度要求。O(log(m+n))的时间复杂度自然是使用二分查找。

与上一题不同，此题的二分对象很明确，就是两个有序数组。但是由于其元素关系并不清楚，二分查找的位置和条件就很不清晰。

我们把寻找中位数转化为找有序数组中第k个数字，其中$k=(m+n)/2$或者$k=(m+n)/2 + 1$。那么，找到A,B两数组中分别的第k/2个数字，对于其中较小的那个：
**两个数组中小于等于它的元素共计不会超过 (k/2-1) + (k/2-1) <= k-2 个，这样其本身最大也只能是第k-1小的元素。
那么小于它的都不可能是第k小的元素。把这些元素全部 "删除"，剩下的作为新的 nums1 数组;nums2同理。**
* 时间复杂度：$O(logk) = O(log((m+n)/2)) = O(log(m+n))$。
* 空间复杂度：O(1)。

## 719_Find_K-th_Smallest_Pair_Distance
此题和287相似，二分查找比较不明显，因为我们将数组排序之后，数对生成的距离也不是有序的，所以不可能把所有的距离写出来排序，再去找第k个。所以二分查找的对象也不是数组本身。

我们明确第k个距离，一定在[0, max_distance]中，就可以对其进行二分。使用count来计数距离小于等于mid的所有数对：
```cpp
if(count < k) low = mid + 1;
else high = mid;
// count该怎么计算？
```
**我们发现，对于当前元素，其距离在左侧有序（即在左侧离它越近的元素和它的数对距离越小）。**
因此，对于某一元素$right$，如果我们找到第一个和它距离小于等于mid的元素$left$，则两者之间的元素到$right$的距离都小于等于mid。因此，可以统计出对于这个元素，和它距离小于等于mid的元素有多少个。遍历整个数组，也就知道所有距离中小于mid的有多少个了。

* 时间复杂度:$O(NlogW + NlogN)$
W为 nums 数组中最大值与最小值的差，即 nums[nums.length - 1] - nums[0]（对 nums 数组进行排序之后）。其中二分查找的时间复杂度为logW，计算count的时间复杂度为O(N)，对nums数组进行排序的时间复杂度为O(NlogN)。

* 空间复杂度O(1)。

## 410_Split_Array_Largest_Sum
类似地，我们想到，二分的对象是划分的数组的最大值:
    $$mid \in [max(nums), \quad sum(nums)]$$

对于某一mid，我们需要验证在划分后最大值满足条件的情况下，划分的组数count和m的关系：
* 如果count > m，那么说明mid肯定太小，必须多切才能满足mid，所以mid应该加大。
    ```low = mid + 1;```
* 如果count < m，那么还可以多切，使mid还可以减小。
    ```high = mid - 1;```
* 如果count = m，那么可以尝试能否多切，使mid还可以减小。
    ```high = mid;```

这样就可以找出在分成m组的情况下，能够得到的最大值的最小值。
* 时间复杂度：$O(n*log(sum - max))$。每次查找后需要遍历一次数组。
* 空间复杂度：O(1)。